    ...
    <div className="shrink-0 px-4 pt-4">
      {label && (
        <div className="my-2 flex items-center gap-1">
          <span className="text-[13px] text-[#9CA3AF]">Type the correct form for</span>
          {label && (
            <span className="inline-flex items-center rounded-[8px] border border-[#E5E7EB] bg-white px-2 py-1 text-[14px] font-semibold text-[#1A1A1A]">
              {label}
            </span>
          )}
        </div>
      ...
      {description && (
        <p className="mt-1.5 text-[13px] leading-normal text-[#9CA3AF]">{description}</p>
      )}
    </div>
  );
};
